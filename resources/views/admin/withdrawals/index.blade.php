...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-1">Manage withdrawal requests from users.</p>
    </div>
</div>
...
<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach ($withdrawals as $w)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $w->user?->name }}</td>
                            <td class="px-4 py-3 text-gray-900 dark:text-white">Rp {{ number_format($w->amount, 0, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('admin.withdrawals.show', $w) }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">Detail</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="text-center py-12 text-gray-500"><p>No withdrawal requests yet.</p></div>
    @endif
</div>
@endsection
